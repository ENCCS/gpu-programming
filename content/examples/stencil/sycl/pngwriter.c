...
void map_colors(double value, const double scaling, const double maxval,
          pixel_t *pix);
    ...
    for (i = 0; i < height; i++) {
        ...
        if (lang == 'c' || lang == 'C') {
            for (j = 0; j < width; j++) {
                ...
                map_colors(data[j + i * width], 2.55, 0.0, &pixel);
                *row++ = pixel.red;
                *row++ = pixel.green;
                *row++ = pixel.blue;
            }
            ...
            for (j = 0; j < width; j++) {
                pixel_t pixel;
                map_colors(data[i + j * height], 2.55, 0.0, &pixel);
                *row++ = pixel.red;
                *row++ = pixel.green;
                *row++ = pixel.blue;
            }
 ...
 */
void map_colors(double value, const double scaling, const double offset,
          pixel_t *pix)
{
    int ival;
