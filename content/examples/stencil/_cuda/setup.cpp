...
#define NSTEPS 500  // iteration steps
    ...
    *nsteps = NSTEPS;
    ...
    switch (argc)
    {
        case 1: // Use default values
            break;
        case 2: // Read initial field from a file
            strncpy(input_file, argv[1], 64);
            read_file = 1;
            break;
        case 3: // Read initial field from a file
            strncpy(input_file, argv[1], 64);
            read_file = 1;
            *nsteps = atoi(argv[2]); // number of time steps
            break;
        case 4:
            rows = atoi(argv[1]); // field dimensions
            cols = atoi(argv[2]);
            *nsteps = atoi(argv[3]); // number of time steps
            break;
        default:
...
// nx = size of the 1st dimension and ny=size of 2nd dimension
...
// Deallocate 2D arrays of heat fields
void finalize(field *heat1, field *heat2)
{
    delete[] heat1->data;
    delete[] heat2->data; 
}
