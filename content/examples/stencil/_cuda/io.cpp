...
void field_write(field *temperature, int iter, parallel_data *parallel)
{
    char filename[64];
...
// Read initial temperature distribution from a file and
// initialize fields for heat1 and heat2 to the same initial state
void field_read(field *temperature1, field *temperature2, char *filename,
                parallel_data *parallel)
{
    ...
    if (parallel->rank == 0)
    {
        full_data = new double [nx * ny]; // full array
    ...
    for (int i = 0; i < nx_local; i++)
        memcpy(&temperature1->data[(i + 1) * (ny_local + 2) + 1], &inner_data[i * ny_local], ny * sizeof(double));
