// main solver routines for heat equation in 2D
...
// Update temperatures using five-point stencil
__global__ void evolve_kernel(double *currdata, double *prevdata,
            double a, double dt, int nx, int ny, double dx2, double dy2)
{

    // Determine temperature field at next time step
    // As we have fixed boundary conditions, the outermost gridpoints are not updated.
    int ind, ip, im, jp, jm;

    // CUDA threads are arranged in column major order
    // thus j index from x and i index from y
...
void evolve(field *curr, field *prev, double a, double dt)
{
    ...
    // arrange CUDA threads in column major order, making ny x nx grid
...
// Copy a temperature field from device to host
...
// Copy a temperature field from host to device
